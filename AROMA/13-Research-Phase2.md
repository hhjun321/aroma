...
| ③ 결함 설계 | 생성할 결함의 뼈대(Edge)와 특징(Metadata) 정의 | Stage 1b seed_profile.json + Stage 2 edge map |
...
result_image = pipe(
    ...
    image=init_image,          # Stage 4 MPB 결과 또는 원본 good 이미지
    mask_image=mask_image,     # Stage 4 *_mask.png (재활용)
    control_image=control_image,  # Stage 2 edge map
...
```
Stage 3 (placement_map) → ROI 좌표 + 회전 정보
    ↓
Stage 4 (Diffusion 합성)
    - image        = Stage 0 원본 good 이미지
    - mask_image   = placement_map 좌표로 생성한 ROI 마스크
    - control_image = Stage 2 edge map
    - prompt       = Stage 1b seed_profile.json (linearity, solidity) → 자연어 변환
    - strength = 0.7 / guidance_scale = 7.5 / num_inference_steps = 30
    ↓
aroma_diffusion 최종 이미지 (MPB 블러 없음)
```
