...
def create_comparison_figure(dataset, roi_aroma_path, roi_random_path,
    ...
    for row, defect_id in enumerate(selected_ids):
        ...
        #   Severstal: "class1_27f44281b" → file: 27f44281b.jpg in test/class1/
        #   MTD: "blowhole_exp3_num_108757" → file: (need to find by ID)
        #   MVTec Leather: "color_016" → file: 016.jpg or similar in test/color/ or test/

        if dataset == "aitex":
            parts = defect_id.replace("defect_", "").split("__")[0]
        elif dataset == "kolektor":
            parts = defect_id.replace("defect_", "")
        elif dataset == "severstal":
            # class1_27f44281b → extract ID only
            parts = defect_id.split("_", 1)[1] if "_" in defect_id else defect_id
        elif dataset == "mtd":
            # blowhole_exp3_num_108757 → search by full ID or decompose
            parts = defect_id  # Try full ID first, then fallback
        else:  # mvtec_leather
            # color_016 → extract number
            parts = defect_id.split("_", 1)[1] if "_" in defect_id else defect_id
        ...
        if baseline_candidates:
            ...
            # Fallback: search for full image_id (some formats may use defect_id directly)
            baseline_candidates = list(test_image_dir.rglob(f"{defect_id}.png")) + list(test_image_dir.rglob(f"{defect_id}.jpg"))
            if baseline_candidates:
                baseline_img = load_image(baseline_candidates[0])
                print(f"    Found baseline (via full ID): {baseline_candidates[0].name}")
            else:
...
def main():
    ...
    for dataset in ["aitex", "kolektor", "severstal", "mtd", "mvtec_leather"]:
        ...
        if dataset == "aitex":
            ...
            test_dir = base_dir / dataset / "test"  # severstal: test/class{1-4}/ structure
        elif dataset == "mtd":
            test_dir = base_dir / dataset / "test"  # mtd: test/type/ (need recursive)
        else:  # mvtec_leather
            test_dir = base_dir / dataset / "test"  # mvtec_leather: test/class/ structure
