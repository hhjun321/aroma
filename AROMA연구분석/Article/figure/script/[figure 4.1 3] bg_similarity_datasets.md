...
입력: `roi/<ds>/clean_bg_selected_ring.json`(AROMA), `roi/<ds>/clean_bg_random_arm_ring.json`(Random), 실 결함 이미지 + 마스크, `train/good` normal. **ring_sgm 재설계 파이프라인 산출물(2026-08-03)** — 구판 `clean_bg_selected.json`(2026-07-13) 기반 수치는 폐기.

## 도출 근거 — 5종 전체 (실측 재계산 2026-08-24, ring 판)
...
**ROI별 값 — `collect(...)`**
각 arm JSON 엔트리에서 **`assigned_normal_id` 단일 필드**만 읽는다(예: Severstal `roi_idx=0` → AROMA `_0db39b46a`, Random `_16585993c`). 선행 `_`를 떼고 `train/good/{stem}.*`를 찾아 **마스크 없이 전체 이미지**의 96차원 텍스처 히스토그램을 만들고, 기준분포와 **histogram intersection**(`Σ min(a,b)`, 0–1)을 취한다.
...
- void 게이트 통과 수·pool 크기·가중치는 `roi/<ds>/clean_bg_summary_ring.md`(파이프라인 산출)에서 인용. pool은 `pool_cut=p95` 적용 결과. void 통과 수·pool 크기는 구판과 동일(void 게이트·p95 컷 불변) — ring 재설계가 바꾼 것은 풀 **내부의 랭킹**(가중치 w_src/w_class/w_k 재산정 + `k_fit` 축 신설 + ring_sgm 자리 선택)뿐이다.

### 3. 데이터셋별 실측 결과

| 데이터셋 | n | AROMA mean±sd | Random mean±sd | Δ | p (단측 MWU) | Cohen's d | 판정 |
|---|---|---|---|---|---|---|---|
| MVTec Leather | 200 | **0.9458** ± 0.0136 | 0.8550 ± 0.0554 | **+0.0909** | 1.1e-51 | **2.25** | `***` |
| AITeX | 200 | **0.5902** ± 0.1051 | 0.5191 ± 0.1162 | **+0.0711** | 2.0e-11 | 0.64 | `***` |
| Severstal | 1,000 | **0.7669** ± 0.0564 | 0.7239 ± 0.0783 | **+0.0430** | 3.8e-43 | 0.63 | `***` |
| Kolektor | 200 | **0.9379** ± 0.0307 | 0.9201 ± 0.0382 | **+0.0179** | 2.0e-06 | 0.51 | `***` |
| MTD | 200 | 0.7686 ± 0.0845 | 0.7580 ± 0.0898 | +0.0106 | 0.103 | 0.12 | **n.s.** |

**"5종 중 4종 유의(MTD만 n.s.)" 주장은 ring 판에서도 재현됨.** Random 평균은 5종 전부 구판과 동일(random arm 풀·시드 불변)이므로 Δ 증가분은 전량 AROMA 선택 개선분이다. 구판 대비: Severstal Δ +0.032→+0.043(d 0.43→0.63), Leather +0.079→+0.091(d 1.87→2.25), Kolektor +0.015→+0.018(d 0.44→0.51), MTD +0.005→+0.011(여전히 n.s.), AITeX 불변(+0.071). 함께 읽어야 할 두 가지:
...
- **효과크기가 데이터셋마다 큰 폭으로 차이난다.** Leather d=2.25(큼) / AITeX 0.64·Severstal 0.63(중) / Kolektor 0.51(중) / MTD 0.12(없음~미미). p값만 보면 4종이 동일하게 `***`로 보이지만, Severstal의 극단적 p(3.8e-43)는 효과크기(d=0.63)와 표본 수(n=1,000)가 함께 만든 값이다.

### 4. 데이터셋별 개별 주석

- **MVTec Leather** — Δ와 d 모두 최대지만 **AROMA가 사용한 distinct 배경이 245장 중 12장**(Random 129장)이다. 유효 풀 243장에 `p95` 컷을 적용해 pool이 13장으로 줄어든 결과이며, 200 ROI가 평균 17회씩 같은 배경을 재사용한다. 즉 "가장 호환적인 배경을 고른다"는 목표는 달성했으나 **배경 다양성을 극단적으로 희생**한 형태다(과거 `img_diversity_cap` 문제와 같은 계열). Leather의 downstream Δ가 null/음수인 것(§4.4)과 함께 읽어야 하며, 이 그림 하나로 Leather에서 AROMA가 유리하다고 서술하면 안 된다.
- **AITeX** — 파이프라인 내부 `hist_intersection`이 mean 0.9175(median 1.0000)로 5종 중 유일하게 높고, 외부 지표와의 상관도 뚜렷한 양(+0.286)이다. `clean_bg_summary_ring.md`의 "aitex 강" 진술과 내·외부 지표가 **함께 일치하는 유일한 데이터셋**. ring 전환으로 배정이 바뀌었는데도 외부 지표는 사실상 불변(0.5903→0.5902) — 구판에서 이미 히스토그램 랭킹이 지배적이던 데이터셋이라 ring 이득이 없다. 절대 수준은 여전히 가장 낮다(AROMA 0.590) — 타일 표면의 텍스처 변동 자체가 커서 기준분포와의 교차가 낮게 나온다. distinct 배경 177장(Random 198장)으로 다양성 희생도 없다.
- **Severstal** — Δ=+0.043, d=0.63. **ring 전환의 최대 수혜자**(구판 +0.032/0.43). 외부 지표와 내부 `hist_intersection`의 상관이 **−0.285(음수)** 라 Δ의 원천이 히스토그램 랭킹(`w_src=0.4492`)이 아니라 `class_fit` 항(`w_class=0.3321`, 외부 상관 +0.347)과 신설 `k_fit` 항(`w_k=0.2188`, 외부 상관 **+0.466**, 5종 최고)쪽이다. distinct 배경 264장(Random 918장).
- **Kolektor** — 유의하나 Δ=+0.018로 최소 수준(구판 +0.015). Random조차 이미 0.920으로 **천장 근처**(good 347장이 균질한 표면)여서 개선 여지 자체가 좁다. AROMA distinct 배경 34장(Random 149장)으로 여기서도 집중이 관찰된다. 결함 이미지가 52장뿐이라 기준분포의 표본 기반도 5종 중 가장 얇다.
- **MTD** — 유일한 n.s.(p=0.103, d=0.12. 구판 p=0.254/d=0.06 — 개선됐으나 여전히 유의 미달). 유효 풀이 956장 중 925장으로 거의 전부 남아 선별 여지가 크지 않고, Random 평균이 이미 0.758이다. 내부 `hist_intersection`도 mean 0.4639로 5종 최저이며 외부 지표와 음의 상관(−0.144). ring fallback도 18.9%로 5종 중 압도적 최고(나머지 0.1–2.6%) — `core_empty`·격자 절단 문제가 겹친 데이터셋. `clean_bg_summary_ring.md`의 "mtd ≈ random" 진술과 일치한다.
...
| 데이터셋 | corr(외부, 내부 `hist_intersection`) | corr(외부, `class_fit`) | corr(외부, `k_fit`) | 내부 `hist_intersection` mean |
|---|---|---|---|---|
| AITeX | **+0.286** | +0.592 | +0.427 | 0.9175 |
| Severstal | **−0.285** | +0.347 | +0.466 | 0.4915 |
| MTD | **−0.144** | +0.505 | +0.445 | 0.4639 |
| Kolektor | **−0.271** | +0.475 | +0.330 | 0.5365 |
| MVTec Leather | **+0.064** | +0.342 | +0.335 | 0.5153 |

  외부 지표는 AROMA가 최대화하는 내부 히스토그램 점수를 되받아 읽는 것이 아니다 — **5종 중 3종에서 음의 상관**이다. 반면 `class_fit`(+0.34 ~ +0.59)과 ring 판에서 신설된 `k_fit`(+0.33 ~ +0.47)은 5종 모두 양의 상관으로, 외부에서 관측되는 우위의 공통 성분은 **`class_fit` + `k_fit` 항**이라는 해석이 데이터와 정합한다. 구판 대비 Δ 증가(특히 Severstal)가 `k_fit` 축 도입(w_k 0.17–0.25)과 동행하는 것도 이와 부합한다.

### 6. `clean_bg_summary_ring.md`의 정직성 배너와의 관계

파이프라인 요약의 배너는 "histogram matching은 도메인 조건부 — aitex 강, severstal/mtd ≈ random"이라고 적고 있다. 이는 **파이프라인 내부 지표(`hist_intersection` lift)** 기준의 진술이고, 본 그림은 **외부 3채널 텍스처 지표**로 다른 것을 잰다. 두 진술은 §5 상관표로 오히려 서로 일관된다 — 내부 히스토그램 랭킹이 강한 곳은 AITeX 하나뿐이고(내부 0.9175, 외부 상관 +0.286), Severstal·Kolektor·MTD에서는 내부 랭킹이 약하거나 외부와 반대 방향이다. **이 그림을 "히스토그램 매칭이 Severstal/Kolektor/Leather에서 강하다"의 근거로 쓰면 안 된다.** 이 그림이 지지하는 명제는 "배경 선택 메커니즘이 균등 추출보다 실 결함 배경에 가까운 배경을 배정한다"까지다.
