# Figure 3.2.4-3 — ROI_score composition (0.6·ctx_prior + 0.4·morph_prior) (spec)

> 2026-08-14: 휴면(`[figure 3.2.5 1]`, PNG 미생성)에서 §3.2.4 Defect Crop Selection 삽화로 부활.
> 경로를 AROMA_DATASET_ROOT 기반으로 수정, 포화-peak 주석 추가. 캡션 정본은 section3_2.txt.
...
## Caption (초안)
**Figure 3.2.4-3.** Composition of ROI_score = 0.6·ctx_prior + 0.4·morph_prior for the top-ranked candidate defect–context pairs on Severstal and AITeX. Each bar splits into the weighted compatibility term (0.6·ctx_prior) and the weighted cluster prior (0.4·morph_prior): compatibility dominates the score and sets the ranking, while the cluster prior adds a small per-cluster offset that orders the compatibility-saturated peaks (ctx_prior = 1).
