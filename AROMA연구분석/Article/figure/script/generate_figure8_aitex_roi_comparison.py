...
def create_comparison_figure(dataset, roi_aroma_path, roi_random_path,
    ...
    for row, defect_id in enumerate(selected_ids):
        ...
        # image_id format: "defect_XXXX_YYY_ZZ__tile_rX_cY"
        # Actual file: XXXX_YYY_ZZ.png in test/YYY/
...
def main():
    ...
    for dataset in ["aitex"]:  # kolektor synth not available
        roi_dir = base_dir / "roi" / dataset
        test_dir = base_dir / dataset / "test"  # Not test/defect; aitex has test/{YYY}/
        synth_aroma_dir = base_dir / "synth_aroma" / dataset / "images"
        synth_random_dir = base_dir / "synth_random" / dataset / "images"

        output_path = Path(f"D:/project/aroma/AROMA연구분석/Article/figures/figure_roi_comparison_{dataset}.png")
