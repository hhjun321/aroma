...
where ctx_prior is read from `matrix_symmetric`, morph_prior is the cluster prior P(k) of §3.2.2, and quality is the crop's photometric quality score (§3.2.6). The top (K) candidates by `ROI_score` become the sources for the placement stages that follow.
