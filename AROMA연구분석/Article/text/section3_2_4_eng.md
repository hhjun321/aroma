...
where ctx_prior is read from `matrix_symmetric` and morph_prior is the cluster prior P(k) of §3.2.2 (unweighted sum — no weighting constants). The top (K) candidates by `ROI_score` become the sources for the placement stages that follow.
