...
All experimental arms composite identical real defect pixels with the identical blending operator and the identical synthesis budget; the placement policy is the only manipulated variable (Introduction, §3.1). This design attributes measured differences specifically to placement, which is the paper's claimed contribution.
