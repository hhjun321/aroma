...
**(2) Systematic sensitivity analysis, followed by parameter elimination.** For the ROI-scoring weights we went further than demonstrating robustness: we removed them. A ctx:morph weight-ratio sweep on all five datasets showed that no ratio from 0.1/0.9 to 0.9/0.1 changes the top-K selection materially (87.5–100% retention), while removing the context term entirely causes substantial divergence (retention falls to 37.5% on Kolektor). Since the ratio is inoperative, the revised Eq. (2) combines the two normalized terms as an unweighted sum — the selection under the unweighted form coincides with the submitted weighted form's selection to 93.5–100% — leaving no weighting parameter to tune. The remaining fixed convention (the tertile percentile positions) is analyzed in the new §4.5.
...
**Manuscript changes:** revised §3.2.2–3.2.3 (data-driven partitions, derived thresholds); Eq. (2) reduced to an unweighted, constant-free combination with a role-based rationale (§3.2.4); new boundary-sensitivity subsection (§4.5); new ablation study (§4.4).
