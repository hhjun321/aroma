...
**(2) Elimination of the remaining manual parameters, and sensitivity analysis of what is left (new §4.5).** The ROI-scoring weights of Eq. (2) — the manually set parameters most directly at issue — have been removed: the revised score is the unweighted sum of two data-derived, normalized priors, so the placement path no longer contains coefficients that could dominate the outcome. The one dataset-independent convention that remains is the tertile partition positions (P33/P66), and for these we provide a systematic sensitivity analysis (§4.5, Table 12): perturbing the boundaries by ±5 percentile points, individually and jointly, relabels only 3.1–14.5% of defects — proportional to the distribution mass moved, with no instability — and cannot alter placements, which do not consume the labels.
...
**Manuscript changes:** revised §3.2.2–3.2.3 (data-driven partitions, Table 4b); Eq. (2) rewritten as the unweighted sum ctx_prior + morph_prior (§3.2.4); new sensitivity subsection (§4.5, Table 12); new ablation study (§4.4, Table 11).
