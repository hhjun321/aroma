...
**(1) The weights have been removed, resolving the contradiction at its source.** The reviewer is right on both counts: the sentence "the ranking introduces no hand-set constants" contradicted the visible 0.6/0.4 weights, and those weights were themselves hand-set. In the revision we removed the weights rather than the claim: Eq. (2) is now the unweighted sum ROI_score = ctx_prior + morph_prior of two data-derived, normalized quantities (a row-normalized compatibility prior and a population-share cluster prior), so the ranking genuinely contains no coefficients. The removal is enabled by the restructured placement pipeline of the revised §3.2.4: the original weighting existed to force the context term to dominate a single composite score, but in the revised three-stage design the compatibility model is consulted directly at every placement stage (ROI selection, background assignment, site resolution), so no priority weighting inside the selection score is needed.

**(2) The demand for justification is redirected to what the revised equation actually contains.** With the weights removed, there is no coefficient left whose optimality could or should be proven; what requires justification is the participation of each term. That justification is empirical: the ablation of §4.4 removes the compatibility-based ROI selection and measures a 3.8 pp mAP drop on Severstal, consistent across all three seeds, identifying the context term as the load-bearing component of the ranking, while the cluster prior's role — keeping the selected pool's cluster mix faithful to the real defect population — is structural and carries no tunable freedom.
...
**Manuscript changes:** Eq. (2) rewritten as the unweighted sum ctx_prior + morph_prior, with rationale, at §3.2.4; new ablation study (§4.4, Table 11); §3.2.6 (Quality Gate, Eq. (4)) removed.
