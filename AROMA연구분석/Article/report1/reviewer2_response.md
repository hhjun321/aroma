...
**(2) The weights in Equation (2) are dataset-independent structural coefficients, not tuning parameters.** They are set once, encode a fixed priority order (context > morphology > quality), and are applied unchanged to all five datasets — no per-dataset adjustment exists anywhere in the pipeline. We note that Eq. (2) has also been corrected to state the full three-component score used by the implementation (0.5·ctx_prior + 0.3·morph_prior + 0.2·quality). To verify that these coefficients do not function as hidden tuning knobs, we added a weight-simplex sensitivity analysis on all five datasets (new §4.5): the top-K selection is stable under ±0.1 perturbations (95.5–100% of selections retained, mean 98.5–100%), while removing any component substantially alters it. The ranking therefore depends on which terms participate, not on the specific coefficient values — the opposite of a manually tuned optimum. As a scale reference, the ablation of §4.4 shows that even replacing the entire scored selection with a uniform-random one shifts mAP by 3.8 pp; the selection differences induced by alternative weight configurations (0–47% of membership) are far smaller in extent. We also emphasize that the correction of Eq. (2) is a correction of the equation's rendering, not a change of method: every result in both the submitted and revised manuscripts was produced by the three-component implementation throughout, and no experiment was re-tuned.

**(3) Equation (4) has been removed, and the quality criterion is disclosed as inherited.** In revising the quality-gating description we deleted the subsection that presented Eq. (4) — its fixed component weights and absolute acceptance threshold — rather than defending it. The underlying gate is a coarse admissibility pre-filter inherited unchanged from our earlier CASDA pipeline; it discards unusable patches *before* any placement decision and is applied identically to the AROMA and random arms, so it is common-mode with respect to every comparison in the paper and is not part of the claimed data-driven contribution. The revised §3.2.4 references the crop quality score with its CASDA provenance where it enters Eq. (2), without restating the formula. No numbered equation followed Eq. (4), so the numbering of Equations (1)–(3) is unchanged.

The revised text (§3.2.2–3.2.4) now states this distinction explicitly: per-dataset parameters are data-derived; the Eq. (2) weights are fixed, dataset-independent design constants whose robustness is demonstrated empirically; and the inherited CASDA quality criterion is disclosed as such.

**Manuscript changes:** Eq. (2) corrected to the three-component score with an explicit fixed-design-choice statement and sensitivity summary (§3.2.4); §3.2.6 (Quality Gate, Eq. (4)) removed, with the quality score's CASDA provenance noted at Eq. (2); new sensitivity subsection (§4.5); data-derived partition descriptions in §3.2.2–3.2.3.
...
**(2) The influence of subtype classification on placement is structurally bounded.** Subtype enters the pipeline only through the quality term of the ROI score (Eq. 2), which carries weight 0.2 and takes a small set of discrete values; a subtype reassignment can therefore shift a candidate's score by at most 0.12. Placement allocation itself operates on the BIC-selected morphology clusters and tertile context cells, not on the subtype labels.

**(3) Threshold sensitivity analysis (new §4.5).** We perturbed the percentile points defining the tertile boundaries by ±5 points (P33/P66 → 28–38 / 61–71) on all five datasets and measured the consequences end-to-end. The response is proportional, with no instability cliff: shifting a single boundary by ±5 points relabels 3.1–9.1% of defects and leaves 67.5–98.0% of the final top-K selection unchanged; shifting both boundaries simultaneously relabels 7.7–14.5% and leaves 64.0–94.5% unchanged — in every case the selection change tracks the distribution mass moved by the boundary shift, rather than amplifying it into a qualitatively different selection. Two complementary results bound the downstream relevance of these membership changes: the weight-simplex sensitivity analysis shows the ranking is stable to the quality term's coefficient (±0.1 → 95.5–100% overlap), and the ablation study (§4.4) shows that what matters for downstream mAP is the presence of the compatibility-based selection mechanism (removing it entirely costs −3.8 pp), not the exact composition of the top-K set.

**Manuscript changes:** §3.2.3 (per-dataset percentile boundaries, Table 4b); new sensitivity subsection (§4.5) reporting the boundary-perturbation results alongside the weight-simplex analysis.
