...
| 실험 | 데이터셋 | 조건 | 지표 |
|------|---------|------|------|
| **exp2** | 5셋(severstal+carpet+leather+macaroni+fryum) | random vs aroma | 선택 ROI 평균 compatibility+quality — **descriptive/sanity 전용**(동어반복: 목적함수=지표), 헤드라인 아님. 구 coverage exp2_execute.md DEPRECATED |
...
### Compounding 평가 (AROMA 핵심, 2026-06-30)
trained ControlNet에 ROI 선택만 swap. dev_note: `aroma_research-core_thesis-and-compounding.md`.
- ✅ **어댑터** `scripts/aroma/aroma_to_casda_roi.py` — AROMA roi_selected.json + morphology_features.csv → CASDA roi_metadata.csv (헤더 byte-exact, severstal 1690 ROI morphology join 100% 검증). py_compile OK.
- ✅ **가이드** `colab_execute/compounding_controlnet_execute.md` — 어댑터 → CASDA `prepare_controlnet_data.py` → `test_controlnet.py`(best_model `controlnet_training_v5.5/best_model`) → `compose_casda_images.py`(Poisson) → exp4v2 4-way severstal. 엔진=CASDA 실제 ControlNet+Poisson(stage4 inpaint 아님), 코어 무수정.
- ✅ CASDA CLI 3종 확인(prepare/test_controlnet/compose_casda_images).
- ✅ **arm 균등화 cap**: 어댑터 `--max_rois` (class-stratified round-robin, 결정론적). **MAX_ROIS=1383** (= CASDA 원본 roi_metadata 행수 = binding constraint; aroma 1690·random 1690을 1383으로 trim → 세 arm 생성풀 동일). compose ×1 → 풀 1383. exp4v2 `--synth_ratio 0.4` → synth_train **1013** (1383≥1013) = 최근 cleanbg 0.4 run과 동급 → 직접 비교. 어댑터 실행 검증: aroma/random/casda 산출 OK.
- ✅ **composed→exp4v2 변환 해소(§3d)**: compose 출력(`images/`+full-frame `masks/`+metadata) → exp4v2 annotations.json (mask_path=full-frame, class=파일명 `_class{N}_`). §3→§4 연결 완성. 가이드 §0→§5 순서 완비.
- 🔲 Colab GPU 실행 (best_model 추론). background_type=complex_pattern, prompt=packager 재생성(in-distribution).
