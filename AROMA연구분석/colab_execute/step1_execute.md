...
> **exp4v2 대상 데이터셋 (v2-1 확정 4종)**: `severstal`(강판)·`mvtec_leather`(가죽)·`aitex`(텍스타일)·`mtd`(자성타일).
> aitex/mtd는 선행으로 `multidomain_integration_verify_execute.md`(다운로드 + prepare + Phase 0 profiling)를 완료해야 한다.
> Step 1→2→3→4로 AROMA 합성본을 만들고, Random 합성(exp3 0단계)까지 준비되면 exp4v2를 실행한다.
