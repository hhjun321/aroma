...
| 인자 | 기본값 | 설명 |
|------|--------|------|
| `--baseline_epochs` | 50 | 전체 조건(baseline/random/aroma) 학습 epoch 수 |
...
| 권장값 | **15** (epochs=50의 30%) — 소규모 데이터(42~70장)는 mAP 곡선 진동이 크므로 ≥10 사용 권장 |
| 비활성화 | `--patience 0` (기본) |
...
> `--patience 15` → best mAP50-95 갱신 없이 15 epoch 경과 시 조기 종료.  
> epochs=50 기준 평균 25~35 epoch에서 종료 예상 (약 30~50% 시간 절약).  
> `--patience 0` (기본) 이면 50 epoch 전부 실행.
...
- **baseline_epochs=50 수렴 가능**: 데이터셋당 real 결함 약 수십~백 장 기준 50 epoch 내 수렴.
