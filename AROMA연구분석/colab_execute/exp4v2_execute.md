...
| `--max_synth_per_ds` | None | synth:real 비율 제어. 권장 128 (real-train의 2배) — **이 128은 mvtec(real_train≈64) 기준. Severstal은 real_train≈2534라 비율 제어는 `--synth_ratio`로 하고 max_synth_per_ds 128을 쓰지 말 것** |
...
**Severstal 기준** (`val_frac=0.3`, real_train ≈ **2534**) — mvtec과 자릿수가 다름:

| `--synth_ratio` | n_synth_cap | 필요 pool(top_k / casda per_class_cap) |
|----------------|------------|----------------------------------------|
| 1.0 (primary) | max(1, int(2534×1.0)) = **2534** | top_k=845(pool 2535) / per_class_cap=243(pool 2538) |
| 1.5 | int(2534×1.5) = **3801** | top_k=1267(pool 3801) / per_class_cap=362 |
| 2.0 | int(2534×2.0) = **5068** | top_k=1690(pool 5070) / per_class_cap=525(pool 5076) |

> 한 번 **top_k=1690 / per_class_cap=525** 로 생성하면 위 3 ratio를 모두 학습 cap 트리밍으로 커버한다. 모든 pool이 cap 이상이라 4조건 등가 budget이 항상 성립.
...
- `--n_per_roi` 동일 → 세 생성기 동일값(=3).
- **CASDA를 동일 pool 규모로 SELECT (결정 B, 갱신)**: 비교의 목적은 ROI-선택 전략(random vs CASDA-suitability vs AROMA-deficit)을 **동일 pool 규모 + 동일 n_per_roi**에서 격리하는 것이다. 따라서 CASDA를 46140(무제한)으로 두고 학습 cap으로 random-trim 하지 않는다 — 대신 CASDA가 **자기 기준(suitability)으로 top-N을 SELECT** 하도록 `--per_class_cap`을 건다.
  - `--per_class_cap X` (uniform int): suitability≥0.5 통과 후 class별 카운트로 X개에서 컷. **c2는 candidate가 ≈117(<X)뿐이라 cap에 안 걸려 전량 선택**(c2-full directive가 코드 변경 없이 충족), c1/c3/c4는 각 X에서 컷 → AROMA와 동일한 c2-full+abundant-scaled 형태.
  - X 산정: pool = (n_c1+n_c2+n_c3+n_c4)×3 ≥ 학습 cap 이 되도록. 2.0까지 스윕(cap=5068) 커버 기준 **X=525**: c2≈117 전량 + 풍부 3class 각 525 → (525+117+525+525)×3 = 1692×3 = **5076 ≥ 5068**. (1:1만이면 X≈243 → (243+117+243+243)×3=846×3=2538 ≥ 2534.) 한 번 X=525로 생성해두면 1.0/1.5/2.0 모두 학습 cap이 trim down.
  - `min_suitability=0.5` 유지(CASDA 기본·전략 정체성). **honest asymmetry**: c2의 117은 suitability 필터 **전** 수치라, c2 중 일부가 <0.5면 CASDA c2가 117보다 약간 작아질 수 있다(AROMA c2는 suitability-gate 없음). 숨기지 말고 `n_synth_per_class`로 드러낼 것. 학습 budget parity는 cap 트리밍으로 어차피 보장된다.
...
> `casda_roi_adapter`가 `min_suitability=0.5`, **`per_class_cap=525`**(결정 B, 동일 pool 규모 SELECT)로 동작. abundant class(c1/c3/c4)는 각 525에서 컷, **c2는 candidate≈117(<525)뿐이라 전량 선택**(c2-full). pool ≈ (≤525×3 + 117)×3 → 학습 cap(2534~5068) 이상이라 trim 후 등가 budget.
> generate_casda → casda_roi_adapter.adapt(per_class_cap=…) 로 정상 전달됨(코드 확인 완료). `--per_class_cap`을 빼면 cap이 무시되어 pool이 46140로 유지된다(학습 budget parity는 여전히 cap 트리밍으로 유지되나, **per-class 형태가 의도와 달라진다** — 반드시 명시할 것).
> n_per_roi=3 은 `$N_PER_ROI`로 전달되며 AROMA/random 생성기와 동일(생성 단계 parity).
> 로그에서 `n_missing_image ≈ 0`과 class별 ROI 수(c3/c4 포함), `n_filtered_cap`(525 cap이 abundant class에 걸린 수)을 확인할 것.
...
# per_class_cap=525: 2.0 스윕(cap=5068)까지 커버. 1:1만이면 243으로도 충분(pool 2538).
# c2는 candidate≈117(<cap)이라 전량 선택 = AROMA와 동일 c2-full 형태.
```

### Cell C — 4조건 exp4v2 실행 (baseline / random / casda / aroma)

> `--condition`은 다중 선택(`nargs="+"`). 일부 조건만 실행하려면 공백으로 나열한다.
> 예: casda 합성이 아직 없으면 `--condition baseline random aroma` (3조건만). `all`이 포함되면 전체로 확장된다.
...
| 항목 | 내용 |
|------|------|
| 조건 | `baseline / random / casda / aroma` (4) — `--condition all` 또는 부분 선택(`--condition baseline random aroma`). 출력은 `ALL_CONDITION_KEYS` canonical 순서로 정렬·중복제거 |
...
>
> #### Severstal pool 규모 + synth_ratio 노트 (load-bearing)
> - `--synth_ratio 1.0`(primary): cap = max(1, int(n_real_train × 1.0)). Severstal n_real_train ≈ **2534**(val_frac=0.3) → cap ≈ **2534**. (mvtec류는 real_train≈64라 cap이 작았지만 Severstal은 훨씬 큼 — 위쪽 `--max_synth_per_ds 권장 128`·synth_ratio 예시표는 mvtec 기준값이니 Severstal엔 그대로 적용 금지.)
> - 모든 synth pool ≥ cap 이어야 4조건 등가 budget: random/aroma pool = top_k(1690)×3 = 5070; casda pool ≈ 5076(per_class_cap=525). 1:1 cap 2534 ≪ 두 pool → 셋 다 정확히 2534로 trim → `n_synth_train` 3조건 동일(아래 (2) parity 셀이 게이트).
> - **스윕**: 1.5(cap=3801)·2.0(cap=5068). pool 5070/5076이 2.0까지 커버하므로 **생성은 1회**(top_k=1690 / per_class_cap=525)로 끝내고 synth_ratio만 바꿔 학습 cap을 trim down. 생성이 비싼 단계, cap 트리밍은 무료.
> - ⚠️ **resume**: synth_ratio만 바꾸면 `--resume`이 기존 severstal 결과를 skip한다. 다른 ratio 재실행 시 `$EXP4V2_OUT/severstal_casda/exp4v2_results.json`(+`_seeds/seed*/`)에서 severstal 삭제하거나 fresh `--output_dir`(예: `severstal_casda_r2.0`) 사용.
> - **무결성**: 선택 **전략**만 조건마다 다르고(그게 실험), pool 규모·n_per_roi(=3)·학습 cap budget은 4조건 동일. AROMA에만 큰 pool을 주거나, CASDA만 random-trim 하거나, c2를 AROMA만 과반복하거나, AROMA가 이기는 ratio만 cherry-pick 하는 것은 rig — 모두 회피. 나온 결과 그대로 보고.
