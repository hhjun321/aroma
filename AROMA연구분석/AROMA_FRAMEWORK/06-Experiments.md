...
> **Claude 요약:** AROMA 평가는 step5가 만든 두 합성 arm(`synth_aroma` = ControlNet + symmetric compat 게이트, `synth_random` = 동일 clean-bg 게이트 통과 통제군)을 소비하여 4개 실험으로 검증한다. 헤드라인은 exp4v2(YOLOv8n supervised detection)로 `baseline`(real만) / `random`(real + random 합성) / `aroma`(real + AROMA-sym 합성) 3조건을 COCO pretrained에서 **fresh 학습**(graft 미사용)하고 mAP50 Δ(aroma−random)를 본다. 보조로 exp3(생성 품질 FID/AD), exp5(PRDC 커버리지), exp6(임베딩 커버리지 knn/rare)가 기제(mechanism) 증거를 제공한다. 실행 순서 체인은 `phase0 → step1~4 → step5(생성) → exp*`이며, 전 실험은 `_SPEC §1` 공통 환경 셀과 `_SPEC §2` stage-first output 규약(`sym_final/{stage}/{ds}`)을 공유한다.
...
- **공통 arm 정의**: `aroma`는 aroma-sym(`generate_defects --method controlnet --compat_mode symmetric` + clean-bg 게이트 + seamless 블렌딩) 산출, `random`은 동일 clean-bg 게이트를 통과한 `generate_random.py` 통제군. 두 arm 모두 `S('synth_aroma')` / `S('synth_random')`에서 **읽기만** 한다(exp에서 재생성 금지 → parity 유지).
...
Random ROI vs AROMA ROI의 2-way 생성 품질을 비교한다. FID는 실제 결함 패치 vs 합성 결함 패치의 분포 거리, AD는 합성본을 train에 섞은 PaDiM 3조건(baseline/random/aroma)의 AUROC. clean-bg 게이트 parity는 step5에서 이미 양 arm 동일 적용 → 남은 차이는 selection + 생성 기여로 해석.
...
외부 임베딩 좌표계(DINOv2)에서 aroma vs random 합성의 real 결함 manifold 커버리지를 반증 가능하게 검증한다(저비용 L2 증거). aroma/random은 동일 clean-bg 게이트 → 차이는 ROI **선택 전략**뿐. 비대칭 예측(Recall만 오르고 Precision 동등)이 사후합리화를 차단.
...
- **합성 재생성 금지 (parity)**: exp는 step5 산출을 읽기만 한다. clean-bg 게이트는 step5에서 양 arm 동일 조건으로 이미 적용됨 — exp에서 게이트를 다시 켜거나 합성을 다시 만들면 parity가 깨진다.
