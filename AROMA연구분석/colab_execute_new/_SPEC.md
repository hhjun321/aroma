...
- **자리 quality 필터 (2026-08-11, 기본 OFF)**: step3.5 `--site_quality_filter --site_quality_pct 15 --image_dir <good>` — ring admissible 자리의 crop-영역 quality 분위 필터(① void 게이트 → ② 자리 quality → ③ ring argmax). 절대 임계 0.7 폐기(leather 100% 포화 실측). 폴백률 영향 ≤+3.5p(5종) → exp4v2 parity 무영향. annotation `site_quality` 전파. 기본 운용 승격 여부는 미결(dev_note `aroma_site_quality_filter.md` §미확정 3). 런타임 `_is_clean_background` 는 여전히 fail-open no-op — 별건.
- **provenance·score 전파 (2026-08-07 개정)**: step3.5 레코드에 `site_mode`/`site_score`/`topk_site_scores`, step5 annotations 에 `position_source`/`site_score`/`bg_score`/`bg_k_fit` 이 추가됐다. exp4v2 는 aroma arm subsample 에서 `position_source=="fallback"` 표본을 **우선 배제**한다(총량 cap 은 arm 간 동일 — parity 불변, 구 산출물은 기존 균일추출과 동일). 발효에는 **step3.5 → step5 재실행 필요**(구 annotations 엔 필드가 없어 legacy 동작). 근거: dev_note `aroma_synth_provenance_and_scores.md`.
