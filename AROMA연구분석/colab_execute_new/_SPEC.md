...
- **생성 선결 준비(step5)**: `build_train_jsonl.py` + `train_controlnet.py`(ControlNet 학습) **+ τ 사전스캔**(`compat_gate_cpu_diagnosis §10`). step4 생성이 소비하는 CN 모델·τ를 이 단계에서 모두 확정. step5는 step0·step3 산출(profiling·roi_candidates)을 입력으로 쓰므로 step3 뒤·step4 앞.
- **실행 순서 체인 = prepare_datasets(step -1) → phase0 → step1 → step2 → step3 → step5(CN 학습+τ) → step4(생성) → exp\***
...
| `roi` | `sym_final/roi/{ds}` | step3 | step4/step5, exp6(rare) |
| `cn_data` | `sym_final/cn_data/{ds}` | step5 | step5 |
| `controlnet_models` | `sym_final/controlnet_models/{ds}/best_model` | step5 | step4 |
| `compat_gate` | `sym_final/compat_gate/{ds}` | step5(τ 사전스캔) | step4 |
| `synth_aroma` | `sym_final/synth_aroma/{ds}` | step4 | exp4v2/exp3/exp5/exp6 |
| `synth_random` | `sym_final/synth_random/{ds}` | step4 | exp4v2/exp3/exp5/exp6 |
...
- **CN 모델**은 `sym_final/controlnet_models/{ds}/best_model`(step5 산출). step4 선결 assert.
...
### step5 — ControlNet 학습 (step4 선결; 순서상 step3 뒤·step4 앞)
```python
# 5a. 학습 데이터 빌드 (CPU)
...
```python
# 5c. τ 사전스캔 (compat_gate_cpu_diagnosis §10) — step4 생성 선결. DS별.
...
```
> τ·AR·TEX는 step4가 소비하므로 **step5 말미에서 모두 확정**한다(생성 직전 재발명 금지).

### step4 — AROMA arm 생성 (controlnet + symmetric)
```python
