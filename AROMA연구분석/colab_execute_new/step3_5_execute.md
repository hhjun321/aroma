...
- **leather 포함 조건**: `mvtec_leather`는 과거 `image_id` stem-collision으로 context_features 조인이 무효였으나, **고유키 수정(`31ee0aa`)을 반영한 phase0 재실행 후에는 정상**이다(dev_note `aroma_phase0_image-id-unique-key.md`). → phase0를 `31ee0aa` 이상으로 재실행했으면 **아래 DATASETS에 leather 포함**(5종, kolektor 포함). 구 profiling만 있으면 leather 제외(4종, kolektor는 포함). STEP 2-2의 `src_match_frac` assert가 혼용을 즉시 잡는다. kolektor는 이 leather 조건과 무관하게 항상 포함한다(domain=mvtec·class_mode=single).
...
> - **(2026-08-11 신설 — 자리 quality 필터, 기본 OFF)** `--site_quality_filter` + `--site_quality_pct 15.0` + `--image_dir <good 디렉터리>` — ring admissible 자리들의 crop-영역 quality(CASDA 4-성분, numpy 구현)를 산출해 **데이터셋 자기 분포의 하위 X% 자리를 배제**한 뒤 생존 자리에서 ring argmax. 절대 임계(0.7) 아님 — leather 처럼 절대값이 포화하는 셋에서도 정확히 X%만 배제된다. void 게이트가 못 보는 **밝기 축**(밝은 침전물)을 보완. 폴백률 영향 실측: kolektor 2.6→2.8%, leather 0.1→0.1% (5종 시뮬레이션 최대 aitex +3.5p — 전부 한계 내). 레코드에 `topk_site_quality`/`site_quality`, `derived`에 `site_quality_floor`/`filtered` 기록. 근거: dev_note `aroma_site_quality_filter.md`
