...
> **⚠️⚠️ 3차 최종 결정 (2026-08-24, article.txt 감사 세션)**: ① **Eq.2 = 무가중 합** `ctx_prior + morph_prior` — 0.6/0.4도 제거 (비율 sweep이 제거 정당화: 어느 비율도 inoperative 87.5–100%, 무가중≈0.6/0.4 선택 93.5–100%). §3.2.4 role-based rationale, §4.5는 경계 섭동 전용, 답변서 R1-1/R2-1/R3-3 "가중치 제거" 문안으로 재작성. ② **CCI 4-성분 유지** — 3-성분 재계산 실험 결과 Kolektor(0.108)/Leather(0.124) 순위 역전으로 §5 단조 서사 붕괴 확인 후 번복·원복. ③ **정본 = Article/report1/article.txt** (조립본) — 인용 1–42 신번호 재생성(YOLO [32] 충돌 → [39], SAM 40→36, 서술자 37/38, FID/KID/LPIPS 40–42), 본문-목록 정합 검증 통과. ④ B군(Abstract 구판, Figure 5 충돌, Table 8 stale ref, void 문장, solidity 0.954, YOLO11n 표기)은 **사용자 지시로 보류** — 착수했다가 전량 원복함. 아래 "최종 결정 A안" 블록은 2차 이력.
>
> **⚠️ 최종 결정 A안 (2026-08-21 세션 말미) — Eq.2는 2-항 표기 유지** (→ 2026-08-24 무가중 합으로 재변경, 위 블록 참조): 세션 중 Eq.2를 3-성분(0.5/0.3/0.2)으로 전환했다가 **최종적으로 제출본 2-항(0.6·ctx + 0.4·morph) 표기로 회귀** (사용자 결정: 수식 단순화 취지 유지, 수식 변경 가시성 리스크 회피). 이에 따라:
