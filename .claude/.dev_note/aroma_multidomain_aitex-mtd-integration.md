...
- **MTD** = GitHub `abin24/Magnetic-tile-defect-datasets` (git clone). 구조: `MT_{Blowhole,Break,Crack,Fray,Uneven,Free}/Imgs/{id}.jpg`+`{id}.png`(mask) co-located.
...
### 2. MTD (자성타일) — domain='mtd'

**원본 레이아웃**(github abin24): `MT_{Blowhole,Break,Crack,Fray,Uneven,Free}/Imgs/` 각 폴더에 `{id}.jpg`(이미지) + `{id}.png`(mask) **co-located**. MT_Free=무결함(952). 결함 392장, 5 defect class.

**통합 방식 (권장 (b) 분기)**:
- **(b) `_find_mask_path` mtd 분기**: mask=이미지와 co-located `{stem}.png`(이미지가 .jpg). config seed_dirs=`MT_{class}/Imgs`, image_dir=`MT_Free/Imgs`. prepare 최소.
- (a) `prepare_mtd.py`: mvtec식 정규화도 가능하나 co-located mask라 분기가 더 단순.
- → **(b) 권장**. 단 MT_Free의 mask(전부 blank/무결함)는 seed 아님 → good로만 사용.

**dataset_config 엔트리**:
```
"mtd": {domain:"mtd", image_dir:".../MTD/MT_Free/Imgs", seed_dirs:[".../MTD/MT_Blowhole/Imgs", ".../MT_Break/Imgs", ".../MT_Crack/Imgs", ".../MT_Fray/Imgs", ".../MT_Uneven/Imgs"], notes:"1344장(free952/defect392), 5 class, co-located {stem}.png mask, jpg+png"}
```
⚠️ MTD 이미지 `.jpg`, mask `.png` 동일 stem → `_find_mask_path` mtd 분기가 `image_path.with_suffix('.png')` 반환. profiling의 이미지 glob이 `.jpg` 포함하는지 확인(§TODO).
...
1. **실제 Drive 하위 폴더명/구조**: AITEX(flat vs 사용자가 어떻게 업로드?), MTD(MT_*/Imgs 유지?). 업로드 후 실경로로 config 확정.
